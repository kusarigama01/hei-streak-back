...
const validateChoices = (choices: { text: string; is_correct: boolean }[]): void => {
...
export const updateQuestionWithChoices = async (
  ...
  const attemptCount = await countAttemptsByExam(question.exam_id);
  if (attemptCount > 0) {
    throw new ApiError(409, "Cannot modify questions of an exam that has attempts");
  }
